...
{
    /**
     * Handle the "creating" event for an EstimateTemplate.
    ...
    /**
     * Handle the "created" event for an EstimateTemplate.
     * Creates associated EstimateTemplateItems and their images from request data.
     */
                ...
                foreach (request()->item_name as $key => $item) {
                    ...
                    if (isset($estimateTemplateItem) && (isset($invoice_item_image[$key]) || isset($invoice_item_image_url[$key]))) {
                        ...
                        if (isset($invoice_item_image_url[$key])) {
                            ...
                            $fileOrgName = ProductFiles::where('product_id', request()->product_id[$key])
                                ->where('hashname', $product->default_image)
                                ->first();

                            $fileName = Files::generateNewFileName($fileOrgName->filename);

                            Files::copy(Product::FILE_PATH . '/' . $fileOrgName->hashname, 
                        ...
                        if (isset($invoice_item_image[$key])) {
                            $filename = Files::uploadLocalOrS3($invoice_item_image[$key], 
                                EstimateTemplateItemImage::FILE_PATH . '/' . $estimateTemplateItem->id . '/');
                        ...
                        $estimateTemplateItemImage->external_link = isset($invoice_item_image[$key]) ? null : ($invoice_item_image_url[$key] ?? null);
    ...
    /**
     * Handle the "updated" event for an EstimateTemplate.
     * Updates existing items, adds new ones, and manages their files.
     *
    ...
    public function updated(EstimateTemplate $estimate)
    {
        if (!isRunningInConsoleOrSeeding()) {
            ...
            if (!empty($request->item_name) && is_array($request->item_name)) {
                // Step1 - Delete items that were removed
                if (!empty($item_ids)) {
                    EstimateTemplateItem::whereNotIn('id', $item_ids)
                        ->where('estimate_template_id', $estimate->id)
                        ->delete();
                }

                // Step2&3 - Update existing items or create new ones
                foreach ($items as $key => $item) {
                    $invoice_item_id = isset($item_ids[$key]) ? $item_ids[$key] : 0;
                    $estimateTemplateItem = EstimateTemplateItem::find($invoice_item_id) ?? new EstimateTemplateItem();
                    ...
                    if ((isset($estimate_item_image[$key]) && $request->hasFile('invoice_item_image.' . $key)) || isset($estimate_item_image_url[$key])) {
                        $estimateTemplateItemImage = EstimateTemplateItemImage::where('estimate_template_item_id', $estimateTemplateItem->id)
                            ->firstOrNew();
                        ...
                        if (!isset($estimate_item_image_url[$key]) && $estimateTemplateItem && $estimateTemplateItem->estimateTemplateItemImage) {
                            Files::deleteFile($estimateTemplateItem->estimateTemplateItemImage->hashname,
                                EstimateTemplateItemImage::FILE_PATH . '/' . $estimateTemplateItem->id . '/');
                        }

                        if (isset($estimate_item_image[$key])) {
                            $filename = Files::uploadLocalOrS3($estimate_item_image[$key],
                                EstimateTemplateItemImage::FILE_PATH . '/' . $estimateTemplateItem->id . '/');
