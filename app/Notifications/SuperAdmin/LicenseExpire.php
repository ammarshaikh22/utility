...
class LicenseExpire extends BaseNotification
{
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ->subject(__('superadmin.licenseExpire.subject') . ' ' . config('app.name')) // Set email subject with app name
            ->greeting(__('email.hello') . ' ' . $notifiable->name) // Personalized greeting with notifiable's name
            ->line(__('superadmin.licenseExpire.text')) // Include license expiration message
